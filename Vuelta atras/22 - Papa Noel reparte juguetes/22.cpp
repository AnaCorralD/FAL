...
struct nodo {
	...
	std::vector<int>& unidadesDisponibles;
	const std::vector<std::string>& tipoJugues;
	const matriz& m;

	std::vector<int> satisfacionActual;
...
bool esSolucionValida(struct nodo& n) {
	...
	for (int i = 0; i < n.satisfacionActual.size(); i++)
		if (n.satisfacionActual[i] < n.info.satisfacion) return false;
	return ok;
}
...
bool esValida(struct nodo& n, int i) {
	...
	if (n.unidadesDisponibles[i] < 0) return false;
	return true;
}
...
void resolver(struct nodo& n) {
	...
	for (int i = 0; i < n.info.juguetes; i++) {
		...
		n.pos[n.k] = i;
		n.unidadesDisponibles[i]--;
		n.satisfacionActual[n.k/2] += n.m[n.k/2][i];
		if (esValida(n, i)) {
			if (esSolucion(n)) {
				...
				nodo siguiente{ n.k + 1, n.info, n.unidadesDisponibles, n.tipoJugues, n.m, n.satisfacionActual, n.cont, n.contJuguestes, n.sol, n.pos };
				resolver(siguiente);
			}
		}
		n.sol[n.k] = -1;
...
bool resuelveCaso() {
	...
	info info;
	std::cin >> info.juguetes >> info.ninos >> info.satisfacion;
	if (!std::cin)
		return false;

	/*Datos*/
	std::vector<int> unidadesDisponibles(info.juguetes);
	for (int i = 0; i < unidadesDisponibles.size(); i++)
		std::cin >> unidadesDisponibles[i];

	std::vector<std::string> tipoJugues(info.juguetes);
	for (int i = 0; i < tipoJugues.size(); i++)
		std::cin >> tipoJugues[i];

	matriz m(info.ninos, std::vector<int>(info.juguetes));
	...
	std::vector<int> satisfacionActual(info.ninos);
	int cont = 0;
	nodo inicio{ 0,info,unidadesDisponibles,tipoJugues,m,satisfacionActual,cont,contJuguetes,sol,pos };
	resolver(inicio);
